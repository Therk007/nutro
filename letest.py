...
bot = telebot.TeleBot('7774669814:AAH3BVOynhCzLFrsmwSanZvbDA462zH2AsU')
           ...
           with open(USER_FILE, "r") as file:
               for line in file:
                   ...
                   if not line:  # Skip empty lines
                       continue
                   try:
                       user_id, attacks, last_reset = line.split(',')
                       user_data[user_id] = {
                           'attacks': int(attacks),
                       ...
                       print(f"Skipping invalid line in {USER_FILE}: {line}")
       except FileNotFoundError:
           print(f"{USER_FILE} not found. Starting with an empty user database.")
...
# Global lock to ensure only one attack runs at a time
...
def handle_attack(message):
    ...
    # Check if the command is sent in the correct group
    ...
    # Acquire the lock to ensure only one attack runs at a time
    ...
    try:
        ...
        # Check if the user has exceeded the attack limit
        ...
        # Validate IP, port, and time duration
        try:
            port = int(port)
        ...
        # Send profile picture and attack start message
        ...
        # Release the lock after the attack is complete
        attack_lock.release()
...
def is_attack_running(user_id):
    ...
    """
    return user_id in pending_feedback and pending_feedback[user_id]
...
👉 [𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝙂𝙧𝙤𝙪𝙥](https://t.me/+OsbZhQwctJNlMWZl) 🚀🔥"""
